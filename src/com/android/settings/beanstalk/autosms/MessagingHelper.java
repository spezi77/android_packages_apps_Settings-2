...
package com.android.settings.beanstalk.autosms;
...
public class MessagingHelper {
    ...
    /* package */ static final String START_SMS_SERVICE =
            "com.android.settings.beanstalk.autosms.START_SMS_SERVICE";
    /* package */ static final String STOP_SMS_SERVICE =
            "com.android.settings.beanstalk.autosms.STOP_SMS_SERVICE";
    ...
    public static int getAutoCallResponseSetting(Context context) {
    ...
    public static int getAutoSmsResponseSetting(Context context) {
        SharedPreferences prefs =
                PreferenceManager.getDefaultSharedPreferences(context);
        return Integer.parseInt(prefs.getString(
    ...
    public static void sendAutoResponse(Context context, String incomingNumber, int setting) {
        String defaultSms = context.getResources().getString(
                R.string.quiet_hours_auto_sms_null);
        SharedPreferences prefs =
                PreferenceManager.getDefaultSharedPreferences(context);
        String message = prefs.getString(KEY_AUTO_SMS_MESSAGE, defaultSms);
    ...
    public static void scheduleService(Context context) {
        ...
        int quietHoursEnd = Settings.System.getInt(context.getContentResolver(),
                Settings.System.QUIET_HOURS_END, 0);
        int autoCall = getAutoCallResponseSetting(context);
        int autoText = getAutoCallResponseSetting(context);

        Intent serviceTriggerIntent = new Intent(context, AutoSmsService.class);
        PendingIntent startIntent = makeServiceIntent(context, START_SMS_SERVICE, 1);
        ...
        if (quietHoursEnd < quietHoursStart) {
            ...
            if (currentMinutes >= quietHoursStart && currentMinutes <= quietHoursEnd) {
                ...
                if (currentMinutes <= quietHoursStart) {
                    ...
                    serviceStartMinutes = MINUTES_PER_DAY - currentMinutes + quietHoursStart;
                    serviceStopMinutes = MINUTES_PER_DAY - currentMinutes + quietHoursEnd;
                }
            }
        }
